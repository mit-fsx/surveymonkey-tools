...
class DateTime:
    """Convenience for TZ conversion"""
    local_tz = pytz.timezone('America/New_York')
    utc_tz = pytz.utc
    default_fmt = '%Y-%m-%d %H:%M:%S'

    def __init__(self, datestring, **kwargs):
        """Initialize with a datestring.
        
        datestring is GMT unless is_local=True is passed
        """
    ...
    def to_utc(self, txt=False):
        ...
        Return a datetime unless txt=True"""
    ...
    def to_local(self, txt=False):
        """Convert the datetime to local TZ
...
class Config(Struct):
    ...
    def get_token(self):
        """Return the token that goes with the config"""
        token = None
        try:
    ...
    def load(filename=None):
        """Load the configuration from the specified file, or
        a default location."""
        if filename is None:
...
class ParsedQuestionResponse:
    ...
    def subheadings(self):
        """Return a list of the subheadings, if any"""
        if self.type == 'open_ended/multi':
...
class OAuth:
    """Convenience class for SurveyMonkey OAuth operations"""
    ...
    def get_and_save_token(self, authorization_code, token_file):
        """Given an OAuth auth code, obtain the long-lived token and
        store it for later.
        """
...
class SurveyMonkey:
    ...
    def get_survey_responses(self, survey_id, *respondents, **kwargs):
        ...
        respondents are RespondentInfo instances, unless by_id is passed
        in which case they are respondent IDs
        """
